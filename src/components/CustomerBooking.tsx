    ...
    try {
      ...
      const { data: inserted, error } = await supabase
        ...
        .insert({
          ...
          // ── TEST MODE — Payment Bypass Active ────────────────────────────────
          // Fygaro is not yet wired up; mark as paid so booking completes instantly.
          // (See src/lib/payments.ts for the future Fygaro hook.)
          payment_status: "paid",
      ...
      // Fetch the 4-digit check-in code generated by the DB trigger so we can
      // display it on the confirmation screen.
      ...
      if (inserted?.id) {
        const { data: fresh } = await supabase
          ...
          .eq("id", inserted.id)
          .maybeSingle();
        checkinCode = (fresh as any)?.checkin_code ?? null;
      }

      // Show confirmation immediately (Test Mode — payment bypassed)
      setConfirmed({
        ...
        <div className="w-full max-w-xs text-center space-y-5">
          ...
          <div className="p-5 rounded-2xl bg-card booka-shadow text-left space-y-3">
            ...
            {confirmed.checkinCode && (
              <div className="pt-2 border-t border-border">
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Check-In Code</p>
                <p
                  data-testid="text-checkin-code"
                  className="text-3xl font-extrabold text-primary font-mono tracking-[0.4em] mt-1 text-center"
                >
                  {confirmed.checkinCode}
                </p>
      ...
      <div className="absolute inset-x-0 top-0 bg-background overflow-y-auto" style={{ bottom: 56, zIndex: 410 }}>
        <div className="flex flex-col items-center justify-start px-6 pt-8 pb-32">
          <div className="w-full max-w-xs space-y-5">
            ...
            <div className="px-4 py-2 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-center">
